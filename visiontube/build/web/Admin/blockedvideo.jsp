<%-- 
    Document   : newjspblockedvideo
    Created on : 9 Feb, 2019, 3:59:36 PM
    Author     : Ketan
--%>
...
<script>
    $(document).ready(function(){
       $("#search").keyup(function(){
           var sd=$("#search").val();
           $.post("blockvidsearch.jsp",{id:sd},function(data){
...
int sn,pg,oset,a;   
    ...
    String videos=null;
                    Class.forName("com.mysql.jdbc.Driver");
                        ...
                        ResultSet rs1=st1.executeQuery("select count(*) from video where status=5");
                        if(rs1.next()){
      ...
      <table class="table" ui-jq="footable" ui-options='{
        ...
        try{                    
                    Class.forName("com.mysql.jdbc.Driver");
                        ...
                        ResultSet rs2=st4.executeQuery("select * from video where status=5 limit 2 offset "+oset+"");
                        while(rs2.next()){
                             %>
                                <tr>
                                <td>
          ...
          <ul class="pagination pagination-sm m-t-none m-b-none">
            <%if(pg>1){
                ...
                if(a>1){
                    a--;
            %> 
                <li><a href="videopage1.jsp?id=<%=a%>"><i class="fa fa-chevron-left"></i></a></li>
            <%
                }
                int pno=1;
                while(pg >=pno){
            %>
            <li><a href="videopage1.jsp?id=<%=pno%>"><%=pno%></a></li>
            <%
                pno=pno+1;
    }
            if(pt<pg){
                pt++;
            %>
             <li><a href="videopage1.jsp?id=<%=pt%>"><i class="fa fa-chevron-right"></i></a></li>
            <%}}%>
          </ul>
        </div>
      </div>
