<%-- 
    Document   : video
    Created on : 7 Feb, 2019, 6:36:00 PM
    Author     : Ketan
--%>
...
<script>
    $(document).ready(function(){
       $("#search").keyup(function(){
           var sd=$("#search").val();
           $.post("vidsearch.jsp",{id:sd},function(data){
    ...
    <div id="sidebar" class="nav-collapse">
        ...
        <div class="leftside-navigation">
            <ul class="sidebar-menu" id="nav-accordion">
                ...
                <li>
                    <a class="active" href="video.jsp?id=1">
...
int sn,pg,oset,a;   
    ...
    String videos=null;
                    Class.forName("com.mysql.jdbc.Driver");
                        ...
                        ResultSet rs1=st1.executeQuery("select count(*) from video where status=1");
                        if(rs1.next()){
                            videos=rs1.getString(1);
                        }
                        cn1.close();
                           int pa=Integer.parseInt(videos);
                           pg=pa/2;
                           if(pa%2 !=0){
                           pg++;
                           }
    ...
    <div>
      <table class="table" ui-jq="footable" ui-options='{
        ...
        try{                    
                    Class.forName("com.mysql.jdbc.Driver");
                        ...
                        ResultSet rs2=st4.executeQuery("select * from video where status=1 limit 2 offset "+oset+"");
                        while(rs2.next()){
                             %>
                                <tr>
                                <td>
                                ...
                                <td>
                                    <a href="blockvideo.jsp?id=<%=rs2.getString("vid")%>">Block<span class="fa fa-ban"></span></a>
                                </td>
      ...
      <div class="row">
        ...
        <div class="col-sm-5 text-center">
          <small class="text-muted inline m-t-sm m-b-sm">showing <%=u%>-<%=sn%> of <%=videos%> Users</small>
        </div>
          ...
          <ul class="pagination pagination-sm m-t-none m-b-none">
            <%if(pg>1){
                ...
                if(a>1){
                    a--;
            %> 
                <li><a href="videopage.jsp?id=<%=a%>"><i class="fa fa-chevron-left"></i></a></li>
            <%
                }
                int pno=1;
                while(pg >=pno){
            %>
            <li><a href="videopage.jsp?id=<%=pno%>"><%=pno%></a></li>
            <%
                pno=pno+1;
    }
            if(pt<pg){
                pt++;
            %>
             <li><a href="videopage.jsp?id=<%=pt%>"><i class="fa fa-chevron-right"></i></a></li>
            <%}}%>
          </ul>
        </div>
      </div>
