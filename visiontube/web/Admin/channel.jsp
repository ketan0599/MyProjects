<%-- 
    Document   : channel
    Created on : 7 Feb, 2019, 5:48:38 PM
    Author     : Ketan
--%>
...
<script>
    $(document).ready(function(){
       $("#search").keyup(function(){
           var sd=$("#search").val();
           $.post("channelsearch.jsp",{id:sd},function(data){
    ...
    <div id="sidebar" class="nav-collapse">
        ...
        <div class="leftside-navigation">
            <ul class="sidebar-menu" id="nav-accordion">
                ...
                <li>
                    <a class="active" href="channel.jsp?id=1">
...
int sn,pg,oset,a;   
    ...
    String channels=null;
                    Class.forName("com.mysql.jdbc.Driver");
                        ...
                        ResultSet rs1=st1.executeQuery("select count(*) from channel where status=0");
                        if(rs1.next()){
                            channels=rs1.getString(1);
                        }
                        cn.close();
                           int pa=Integer.parseInt(channels);
                           pg=pa/2;
                           if(pa%2 !=0){
                           pg++;
                           }
    ...
    <div>
      <table class="table" ui-jq="footable" ui-options='{
        ...
        try{                    
                    Class.forName("com.mysql.jdbc.Driver");
                        ...
                        ResultSet rs2=st4.executeQuery("select * from channel where status=0 limit 2 offset "+oset+"");
                        while(rs2.next()){
                             %>
                                <tr>
                                <td>
                                ...
                                <td>
                                    <a href="blockchannel.jsp?id=<%=rs2.getString("cid")%>">Block<span class="fa fa-ban"></span></a>
                                </td>
      ...
      <div class="row">
        ...
        <div class="col-sm-5 text-center">
          <small class="text-muted inline m-t-sm m-b-sm">showing <%=u%>-<%=sn%> of <%=channels%> Users</small>
        </div>
          ...
          <ul class="pagination pagination-sm m-t-none m-b-none">
            <%if(pg>1){
                ...
                if(a>1){
                    a--;
            %> 
                <li><a href="channelpage.jsp?id=<%=a%>"><i class="fa fa-chevron-left"></i></a></li>
            <%
                }
                int pno=1;
                while(pg >=pno){
            %>
            <li><a href="channelpage.jsp?id=<%=pno%>"><%=pno%></a></li>
            <%
                pno=pno+1;
    }
            if(pt<pg){
                pt++;
            %>
             <li><a href="channelpage.jsp?id=<%=pt%>"><i class="fa fa-chevron-right"></i></a></li>
            <%}}%>
          </ul>
        </div>
      </div>
